...
// make sense. Super does not make a lot of sense, but complex terminal applications
// commonly support it.
...
pub const Input = struct {
    pub const fromDescriptor = @import("input_descriptor.zig").parseInputDescriptor;

    /// Checks whether the Input equals an input descriptor. Descriptor must
    /// be comptime known.
    pub fn eqlDescriptor(self: Input, comptime descr: []const u8) bool {
        const descriptor_input = comptime Input.fromDescriptor(descr) catch @compileError("atleg: Bad input descriptor.");
        return meta.eql(self, descriptor_input);
    }
