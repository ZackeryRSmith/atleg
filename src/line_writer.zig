...
pub fn LineWriter(comptime UnderlyingWriter: type) type {
    return struct {
        const Self = @This();
        ...
        pub fn padByte(self: *Self, b: u8) !void {
            try self.finish();
            try self.underlying_writer.writeByteNTimes(b, self.len_left);
            self.len_left = 0;
        }

        pub fn padAmount(self: *Self, amount: usize) !void {
            try self.finish();
            try self.underlying_writer.writeByteNTimes(' ', amount);
            self.len_left -= amount;
        }

        pub fn padAmountByte(self: *Self, amount: usize, b: u8) !void {
            try self.finish();
            try self.underlying_writer.writeByteNTimes(b, amount);
            self.len_left -= amount;
        }
...
pub fn lineWriter(underlying_stream: anytype, len: usize) LineWriter(@TypeOf(underlying_stream)) {
    return .{
...
test "LineWriter" {
    ...
    const testing = std.testing;
    {
        ...
        var rpw = lineWriter(counting_writer.writer(), bytes.len);
    ...
    }
    {
        const bytes = "12345678";
        var counting_writer = io.countingWriter(io.null_writer);
        var rpw = lineWriter(counting_writer.writer(), bytes.len - 1);
        try rpw.writer().writeAll(bytes);
        try rpw.finish();
    ...
    }
    {
        const bytes = "12345678";
        var counting_writer = io.countingWriter(io.null_writer);
        var rpw = lineWriter(counting_writer.writer(), 20);
        try rpw.writer().writeAll(bytes);
        try rpw.pad();
